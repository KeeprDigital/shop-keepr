...
import { CatalogueRequestError, createCatalogueClient, FIRST_CURSOR } from '../../../server/catalogue/client';
...
function client(fetch: typeof globalThis.fetch = fixtureFetch({ baseURL, credential })) {
...
describe('catalogue client', () => {
	it('walks a Game System from cursor zero to the end', async () => {
		...
		const first = await catalogue.walkCatalogue('magic', FIRST_CURSOR);
	...
	it('walks Market Price movements on their own cursor', async () => {
		const page = await client().walkMarketPrices('pokemon', FIRST_CURSOR);
	...
	it('identifies itself with the credential', async () => {
		const wrongKey = createCatalogueClient({ fetch: fixtureFetch({ baseURL, credential }), baseURL, credential: 'someone-else' });
		await expect(wrongKey.walkCatalogue('magic', FIRST_CURSOR)).rejects.toBeInstanceOf(CatalogueRequestError);
		await expect(wrongKey.walkCatalogue('magic', FIRST_CURSOR)).rejects.toMatchObject({ status: 401, problem: { title: 'authentication_required' } });
	});

	it('surfaces a failed request with its status', async () => {
		await expect(client().walkCatalogue('no-such-game', FIRST_CURSOR)).rejects.toMatchObject({ status: 404 });
	});
	...
	it('hands back a record that fails validation with its raw payload, and keeps the rest', async () => {
		...
		const page = await client(async () => Response.json({ records: [broken, set], next_cursor: 'x-0002', has_more: false })).walkCatalogue('x', FIRST_CURSOR);
	...
	it('refuses a page whose envelope is not the contract', async () => {
		await expect(client(async () => Response.json({ items: [] })).walkCatalogue('x', FIRST_CURSOR)).rejects.toBeInstanceOf(CatalogueRequestError);
	});
});
