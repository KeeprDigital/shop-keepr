...
describe('inventory (spec §8.2, `/inventory`)', () => {
	...
	it('lists every SKU with stock on hand, with its Printing, Condition, Language, on-hand and withdrawn flag', async () => {
		...
		expect(rows[1]).toMatchObject({ printingId: 'prt-pokemon-base1-4', gameSystem: 'pokemon', setCode: 'base1', collectorNumber: '4', finish: 'holo' });
		expect(rows[1]!.skuId).toBeTypeOf('string');
	});

	it('filters by Game System and by name', async () => {
		expect((await listInventory(db, { game: 'magic' })).rows.map(row => row.name)).toEqual(['Black Lotus']);
	...
	it('sorts by any column, in either direction', async () => {
		expect((await listInventory(db, { sort: 'onHand', direction: 'desc' })).rows.map(row => row.onHand)).toEqual([4, 2, 1, 1]);
		expect((await listInventory(db, { sort: 'condition', direction: 'asc' })).rows.map(row => row.condition)).toEqual(['NM', 'NM', 'LP', 'MP']);
		expect((await listInventory(db, { sort: 'language', direction: 'desc' })).rows.map(row => row.language)).toEqual(['ja', 'en', 'en', 'en']);
	});
});
