...
**Transaction** — one customer settled with at the till in one act: cards going into or out of the system, one POS Reference, one **net**. Always stated **from the store's perspective**. A Transaction is recorded as one or two ledger **entries**, and every entry is one of two types:
...
**Trade** — a Buy and a Sell settled with one customer in one act, where the money that moves is the difference. A Trade is not a third kind of entry: it is one Transaction recorded as a Buy entry and a Sell entry together, each with its own Total and both carrying the same net, the difference ([#11](https://github.com/KeeprDigital/shop-keepr/issues/11), [ADR 0007](https://github.com/KeeprDigital/shop-keepr/blob/main/docs/adr/0007-trade-is-a-linked-buy-and-sell.md)). Whether the Transaction being built at the counter commits as a Buy, a Sell or a Trade depends only on which sides have cards in it. Trading cards is dealing in credit: the part of the Buy that pays for the Sell, the **Covered** part, is always store credit, and whatever the customer's cards are worth beyond that is the **Remainder**, paid in a Tender staff choose ([#43](https://github.com/KeeprDigital/shop-keepr/issues/43)). When the Sell is worth more than the cards, the customer pays the difference and shop-keepr does not record how ([#44](https://github.com/KeeprDigital/shop-keepr/issues/44)).
...
**Buy Price** — what the store pays for a SKU. Derived from the Printing's Market Price by the store's **Pricing Rules**, independently of the Sell Price and with its own settings throughout. Quoted for any Printing, including ones the store has never held. Unlike the Sell Price it also depends on how many copies the store holds and how many are on the line, through Stock Bands and Quantity Bands, and never falls below the Bulk Price ([#48](https://github.com/KeeprDigital/shop-keepr/issues/48)). Always quoted in the store's **Default Tender**; the other Tender is never a price on a line, only a factor on a Buy's total.

**Pricing Rules** — the store's own rules for turning a Market Price into a Sell Price and a Buy Price. They are the store's commercial policy, not a property of any card: the store decides what a Lightly Played copy is worth to it, what it pays for a rare, and what it will never sell below. Rules may be set per Game System, and any rule a Game System does not set falls back to the store's defaults ([#8](https://github.com/KeeprDigital/shop-keepr/issues/8), [ADR 0004](https://github.com/KeeprDigital/shop-keepr/blob/main/docs/adr/0004-configurable-pricing-pipeline.md)). Every step of a rule is **a multiplier and a flat amount**: per Condition, per Language, per Value Band, per Stock Band and Quantity Band, and per Pricing Attribute value ([#42](https://github.com/KeeprDigital/shop-keepr/issues/42)).
_Avoid_: modifier for the flat amount (retired: it is part of every step, not a thing of its own). **Tender Modifier** is a different term and stands.
...
**Value Band** — a range of Market Price, after currency conversion, to which a Pricing Rule attaches the percentage a card in that range is priced at: cheap cards and expensive cards take different percentages. Keyed on the Market Price, never on the price being calculated ([#8](https://github.com/KeeprDigital/shop-keepr/issues/8), [#42](https://github.com/KeeprDigital/shop-keepr/issues/42)).

**Bulk Price** — the lowest the store ever pays for a card: the Buy-side Floor set per Game System, named for what it does. The store never refuses a card by rule; where its Pricing Rules would pay less, it offers the Bulk Price ([#48](https://github.com/KeeprDigital/shop-keepr/issues/48)). A per-Pricing-Attribute Buy Floor may sit above it.

**Stock Band** — a range of copies of a SKU the store already holds, to which a Buy Pricing Rule attaches a multiplier and a flat amount: the more of a card the store has, the less it pays for another. Keyed on stock on hand before the Buy, never on what is held in Baskets. Buy side only ([#48](https://github.com/KeeprDigital/shop-keepr/issues/48)).

**Quantity Band** — a range of copies of a SKU on one line of a Buy, to which a Buy Pricing Rule attaches a multiplier and a flat amount: seven of one card attract a lower rate than one, whatever the store holds. The whole line takes one price. Buy side only ([#48](https://github.com/KeeprDigital/shop-keepr/issues/48)).
