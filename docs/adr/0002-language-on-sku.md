...
**Language becomes a pricing rule input.** Market Price attaches to the Printing and knows nothing about language, while a Japanese card is often worth materially more or less than its English equivalent — so an English and a Japanese copy of one Printing start from the same rate and can only diverge downstream. Language therefore joins Condition as an input to the Sell and Buy rules ([#21](https://github.com/KeeprDigital/shop-keepr/issues/21)), with the multiplier itself sized by [#8](https://github.com/KeeprDigital/shop-keepr/issues/8). Where the rule still gets it wrong, staff fix the price with a Pinned Price.
