...
A Sell or Buy Price is produced by folding an **ordered list of steps** over the Market Price, where both the steps' values and their order are store configuration. The steps come from a closed set — `x FX`, `x condition multiplier`, `x language multiplier`, `x percentage`, `+ modifiers`, `round`, `floor`, and on the Buy side only `x stock multiplier` and `x quantity multiplier` — and every value in them resolves down a chain: attribute row, then Game System rules, then store defaults. Decided in [#8](https://github.com/KeeprDigital/shop-keepr/issues/8); the two Buy-side steps added in [#48](https://github.com/KeeprDigital/shop-keepr/issues/48).
